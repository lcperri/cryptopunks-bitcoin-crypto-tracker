...
import { Airplay, Search } from 'lucide-react';
  ...
  return (
    ...
    py-5 bg-gray-900/80 backdrop-blur-md border-b border-gray-700/30 sticky top-0 z-50 '>
      ...
      transition-transform'>
        <Airplay className=' w-8 h-8 text-emerald-400 drop-shadow-[0_0_8px_rgba(74,222,128,0.6)] '/>
        <span className=' text-xl font-bold bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text
        text-transparent '>
          Monitor de Bitcoin & Crypto
        </span>
      </a>

      {/* SEARCH */}
      <form onSubmit={searchHandler} className=' order-3 w-full md:order-2 md:w-auto flex-1, max-w-2x1 mx-0 md:mx-4 relative '>
        ...
        { filteredCoins.length > 0 && (
          <ul className=' absolute w-full bg-gray-800/95 border border-gray-700 mt-2 rounded-lg shadow-xl z-10 backdrop-blur-md '>
            { filteredCoins.map((coin, index) => (
                <li key={index} className=' px-4 py-2 hover:bg-emerald-600/30 cursor-pointer text-gray-100 '
                  onClick={() => {
                    setInput(coin.name);
                    setFilteredCoins([]);
                  }}>
